...
macro(include_target_parameters_of __TEMPLATE_NAME)
	...
	elseif("${__IS_INSIDE_TARGETS_FILE}" STREQUAL 1)
		...
		_append_postprocessing_option("${CMAKE_PARENT_LIST_FILE}" INCLUDE_TEMPLATE_NAME "${__TEMPLATE_NAME}")
...
function(_append_postprocessing_option __TARGETS_CMAKE_PATH __ACTION __ACTION_PARS)
	message(STATUS "_append_postprocessing_option(): __ACTION_PARS: ${__ACTION_PARS}")
	message(STATUS "_append_postprocessing_option(): _parse_postprocess(__PPPARS ${__ACTION} ${__ACTION_PARS})")
	_parse_postprocess(__PPPARS ${__ACTION} ${__ACTION_PARS})
	...
	message(STATUS "_append_postprocessing_option(): __HASH: ${__HASH} __HASH_SOURCE: ${__HASH_SOURCE}")
	if(NOT "${__HASH}" IN_LIST __POSTPROCESS__LIST)
...
function(_read_targets_file __TARGETS_CMAKE_PATH __SKIP_RECURRENCE __OUT_READ_PREFIX __OUT_IS_TARGET_FIXED)
	...
	set(__POSTPROCESS__LIST) #Clear all possible previous postprocessing
...
macro(_process_all_postprocessing __REF_TARGETS_CMAKE_PATH)
	foreach(__PP_HASH IN LISTS __POSTPROCESS__LIST)
...
macro(_parse_postprocess __OUT_PREFIX __ACTION)
	if("${__ACTION}" STREQUAL "INCLUDE_TEMPLATE_NAME")
		...
		message(FATAL_ERROR "Internal beetroot error: unknown postprocessing action: ${__ACTION} in ${CMAKE_PARENT_LIST_FILE}.")
	endif()
#	message(STATUS "_parse_postprocess(): ARGN: ${ARGN}")
...
function(_process_postprocess __REF_TARGETS_CMAKE_PATH __ACTION)
	_parse_postprocess(__PPPARS ${__ACTION} ${ARGN})
	if("${__ACTION}" STREQUAL "INCLUDE_TEMPLATE_NAME")
		...
		#We need to stash current TARGET_PARAMETERS
...
macro(_parse_all_postprocessing __POSTPROCESSING __OUT_PREFIX)
	set(__OPTIONS )
	set(__oneValueArgs )
	set(__multiValueArgs INCLUDE_TARGET_PARAMETERS)
	
	cmake_parse_arguments(${__OUT_PREFIX} "${__OPTIONS}" "${__oneValueArgs}" "${__multiValueArgs}" ${${__POSTPROCESSING}})
	set(__unparsed ${__PARSED_UNPARSED_ARGUMENTS})
	if(__unparsed)
		message(FATAL_ERROR "Internal beetroot error: Undefined postprocessing for targets file: ${__unparsed}. All options: ${__POSTPROCESSING}")
	endif()
endmacro()

function(_parse_postprocessing __PP __TARGETS_CMAKE_PATH __PROPERTY __OUT)
	_parse_all_postprocessing(${__PP} ___P)
#	message(STATUS "_parse_external_info(): __EI: ${__EI}: ${${__EI}}")
	if(NOT ${__PROPERTY} IN_LIST __OPTIONS AND NOT ${__PROPERTY} IN_LIST __oneValueArgs AND NOT ${__PROPERTY} IN_LIST __multiValueArgs)
		message(FATAL_ERROR "Internal Beetroot error: property name ${__PROPERTY} is not valid for of preprocessing options in file ${__TARGETS_CMAKE_PATH}")
	endif()

...
#	endif()
#	message(STATUS "_get_variables(): __ARGS_IN: ${__ARGS_IN}: ${${__ARGS_IN}__LIST} __TARGETS_CMAKE_PATH: ${__TARGETS_CMAKE_PATH}")
...
#			message(STATUS "_get_variables(): Veryfying variable ${__VAR_NAME}")
			...
			_verify_parameter("${__VAR_NAME}" "${__SRC}" "${__PARS_${__VAR_NAME}__CONTAINER}" "${__PARS_${__VAR_NAME}__TYPE}" "${__ARGS_${__VAR_NAME}}" ${__BOOL_FEATURES} "${__TARGETS_CMAKE_PATH}")
...
#When IS_FEATURE is set, there is smaller set of valid type+container combinations
	...
	if(NOT "${TYPE}" MATCHES "^CHOICE\(.+\)$" AND NOT "${TYPE}" IN_LIST VALID_TYPES)
		message(FATAL_ERROR "Wrong type for variable ${NAME} ${CONTEXT} in ${__TARGETS_CMAKE_PATH}. Must be INTEGER, PATH, STRING or CHOICE(opt1,opt2,...,optN) format, but got ${TYPE}")
	endif()
	...
	if("${CONTAINER}" STREQUAL "OPTION")
		...
		_verify_value("${NAME}" "${CONTEXT}" BOOL "${VALUE}" ${IS_FEATURE} "${__TARGETS_CMAKE_PATH}")
	else()
		foreach(VAL IN LISTS VALUE)
			_verify_value("${NAME}" "${CONTEXT}" "${TYPE}" "${VAL}" ${IS_FEATURE} "${__TARGETS_CMAKE_PATH}")
		endforeach()
	endif()
endfunction()

function(_verify_value NAME CONTEXT TYPE VALUE IS_FEATURE __TARGETS_CMAKE_PATH)
...
function(_read_variables_from_args __PARS __ARGS __OUT_ARGS)
	set(__OPTIONS)
	set(__oneValueArgs )
...
#	message(STATUS "_read_variables_from_args(): __multiValueArgs:${__multiValueArgs} __oneValueArgs: ${__oneValueArgs}")
...
#	message(STATUS "_read_variables_from_args(): __OUT_ARGS: ${__OUT_ARGS} __PARSED_WHO: ${__PARSED_WHO} __IN_ARGS_WHO: ${__IN_ARGS_WHO} ${__ARGS}__LIST: ${${__ARGS}__LIST} ")

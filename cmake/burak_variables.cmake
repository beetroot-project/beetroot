...
#		if(__ARGS_VERSION)
#			message(STATUS "_get_variables(): __ITERATION: ${__ITERATION}, phase 3, __ARGS_VERSION: ${__ARGS_VERSION} ARGN: ${ARGN}")
#		endif()
...
function(_re_read_variables __TARGETS_CMAKE_PATH __ARGS_IN __OUT_ARGS __OUT_EXTERNAL_PROJECT_INFO)
	set(__ARGUMENT_HASH)
	set(__ITERATION_COUNT 0)
...
#		endif()
		...
		message(STATUS "_re_read_variables(): __ARGUMENT_HASH_OLD: ${__ARGUMENT_HASH_OLD}, __ARGUMENT_HASH_NEW: ${__ARGUMENT_HASH_NEW}, __IN_EXTERNAL_PROJECT_INFO: ${__IN_EXTERNAL_PROJECT_INFO}")
		if("${__ARGUMENT_HASH_NEW}" STREQUAL "${__ARGUMENT_HASH_OLD}")
			break()
		endif()
		set(__ARGUMENT_HASH_OLD "${__ARGUMENT_HASH_NEW}")
	...
	_pass_arguments_higher(${__OUT_ARGS} ${__ARGS})
	set(${__OUT_EXTERNAL_PROJECT_INFO} "${__IN_EXTERNAL_PROJECT_INFO}" PARENT_SCOPE)
endfunction()
...
#	message(FATAL_ERROR "${__OUT_PARAMETERS_PREFIX}_WYBOR__CONTAINER: ${${__OUT_PARAMETERS_PREFIX}_WYBOR__CONTAINER}")
	...
	set(${__OUT_GLOBAL_OPTIONS} "${__READ_TEMPLATE_OPTIONS}" PARENT_SCOPE)
endfunction()
...
function(_join_variables __ARGS1 __ARGS2 __ERR_MSG __OUT_ARGS)
	list_union(__UNION __ARGS1 __ARGS2)
	foreach(__VAR IN LISTS __UNION)
		if("${__VAR}" IN_LIST ${__ARGS1})
			if("${__VAR}" IN_LIST ${__ARGS2})
				if(NOT "${${__ARGS1}_${__VAR}}" STREQUAL "${${__ARGS2}_${__VAR}}")
					message(FATAL_ERROR "${__ERR_MSG}. Parameter ${__VAR} set to both \"${${__ARGS1}_${__VAR}}\" and \"${${__ARGS2}_${__VAR}}\"")
				endif()
			endif()
			set(${__OUT_ARGS} "${${__ARGS1}_${__VAR}}" PARENT_SCOPE)
		else()
			if("${__VAR}" IN_LIST ${__ARGS2})
				set(${__OUT_ARGS} "${${__ARGS2}_${__VAR}}" PARENT_SCOPE)
			else()
				message(FATAL_ERROR "Internal beetroot error. Something wrong with list_union function.")
			endif()
		endif()
	endforeach()
	set(${__OUT_ARGS}__LIST ${__UNION} PARENT_SCOPE)
endfunction()
...
#	message(STATUS "_make_instance_name(): called for __INSTANCE_ID: ${__INSTANCE_ID}")
	...
	if(__INSTANCE_NAME)
#		message(STATUS "_make_instance_name(): __INSTANCE_ID ${__INSTANCE_ID} already got a name ${__INSTANCE_NAME}")
...
#	message(STATUS "_make_instance_name(): __ARGS__LIST_MODIFIERS: ${__ARGS__LIST_MODIFIERS}")
...
#			message(STATUS "_make_instance_name(): __HASH ${__HASH} already in list of assignet targets with number ")
		else()
			if(__IS_TARGET_FIXED)
				message(FATAL_ERROR "Target ${__TEMPLATE_NAME} is fixed, which means you can't have two versions of ${__TEMPLATE_NAME} in one build. Right now we have ${__NUMBER_OF_TARGETS} versions. ")
			endif()
...
#		message(STATUS "_make_instance_name(): __NUMBER_OF_TARGETS equas ${__NUMBER_OF_TARGETS}" )

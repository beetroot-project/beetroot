			...
			if(__IS_COMPATIBLE)
				if("${__COMP_FB_ID}" STREQUAL "")
                	...
                	_retrieve_instance_data(${__INSTANCE_ID} I_PARENTS __I_PARENTS)
                 	...
                 	_get_nice_featurebase_name("${__COMP_FB_ID}" __FIRST_NICE_FEATUREBASE)
                 	_get_nice_featurebase_name("${__FEATUREBASE_ID}" __SECOND_NICE_FEATUREBASE)
        			nice_list_output(LIST "${__ALL_PARS}" OUTVAR __ALL_PARS_NICE)
					message(FATAL_ERROR "Beetroot error: Virtual dependency ${__NICE_INSTANCE_ID} (required by ${__NICE_PARENTS_ID}) can be successfully instantiated by at least two targets: ${__FIRST_NICE_FEATUREBASE} and ${__SECOND_NICE_FEATUREBASE} based on match on the following parameters: ${__ALL_PARS_NICE}. Please use target parameters in such a way as to uniqually fit this dependency to exactly one target, or remove the conflicting targets (or combination of both actions).")
				endif()
			else()

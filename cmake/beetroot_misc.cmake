#Returns base name for the external project.
function(_find_base_name __INSTANCE_ID __OUT_BASE_NAME)
   ...
   #get_filename_component(__TEMPLATE_DIR ${__PARSED_SOURCE_PATH} DIRECTORY)
   if(NOT __PARSED_NAME)
      if("${__PARSED_SOURCE_PATH}" STREQUAL "")
         _retrieve_instance_data(${__INSTANCE_ID} PATH __TARGETS_CMAKE_PATH) 
         get_filename_component(__EXTERNAL_BARE_NAME ${__TARGETS_CMAKE_PATH} NAME_WE)
#      	message(STATUS "${__PADDING}_find_base_name() __EXTERNAL_BARE_NAME from __TARGETS_CMAKE_PATH ${__TARGETS_CMAKE_PATH}: ${__EXTERNAL_BARE_NAME}")
      else()
         get_filename_component(__EXTERNAL_BARE_NAME ${__PARSED_SOURCE_PATH} NAME_WE)
#      	message(STATUS "${__PADDING}_find_base_name() __EXTERNAL_BARE_NAME from __PARSED_SOURCE_PATH ${__PARSED_SOURCE_PATH}: ${__EXTERNAL_BARE_NAME}")
      endif()
   else()
      set(__EXTERNAL_BARE_NAME "${__PARSED_NAME}")
#   	message(STATUS "${__PADDING}_find_base_name() __EXTERNAL_BARE_NAME from external info: ${__PARSED_NAME}")
   endif()
   set(${__OUT_BASE_NAME} ${__EXTERNAL_BARE_NAME} PARENT_SCOPE)
endfunction()

#function(_find_install_dir __INSTANCE_ID __OUT_INSTALL_DIR)
#   _retrieve_instance_data(${__INSTANCE_ID} INSTALL_DIR __INSTALL_DIR)
#   if("${__INSTALL_DIR}" STREQUAL "")
...
macro(_populate_implicit_variables_for_file __FILE_HASH)
   _retrieve_file_data(${__FILE_HASH} EXTERNAL_INFO __EXTERNAL_INFO__LIST)
   _parse_all_external_info(__EXTERNAL_INFO "EP")
   _retrieve_file_data(${__FILE_HASH} FILE_OPTIONS __FILE_OPTIONS__LIST)
#   message(WARNING "_populate_implicit_variables_for_file(): __FILE_OPTIONS__LIST: ${__FILE_OPTIONS__LIST}")
   _parse_all_file_options(__FILE_OPTIONS "FO")
endmacro()

function(_make_build_version_string __FILE_HASH __OUT_FUNCTION_EXISTS __OUT_VERSION_STRING)
	_retrieve_file_data(${__FILE_HASH} BUILD_VERSION __VERSION_STRING)
	if(NOT "${__VERSION_STRING}" STREQUAL "")
   	if("${__VERSION_STRING}" STREQUAL "  <#UNDEFINED_FUNCTION#>  ")
		   set(${__OUT_FUNCTION_EXISTS} 0 PARENT_SCOPE)
	      set(${__OUT_VERSION_STRING} "" PARENT_SCOPE)
      else()
		   set(${__OUT_FUNCTION_EXISTS} 1 PARENT_SCOPE)
	      set(${__OUT_VERSION_STRING} "${__VERSION_STRING}" PARENT_SCOPE)
   	endif()
      return()
	endif()
	_populate_implicit_variables_for_file(${__FILE_HASH})
	_retrieve_file_data(${__FILE_HASH} PATH __TARGETS_CMAKE_PATH)
#	set(__TARGETS_CMAKE_PATH "${FO_PATH}")
#	_retrieve_file_data(${__FILE_HASH} EXTERNAL_INFO __EXTERNAL_INFO__LIST)
##	message(STATUS "${__PADDING}_make_build_version_string(): __EXTERNAL_INFO: ${__EXTERNAL_INFO}")
#   _parse_all_external_info(__EXTERNAL_INFO "EP")
##	message(STATUS "${__PADDING}_make_build_version_string(): EP_SOURCE_PATH: ${EP_SOURCE_PATH}")
	if("${__TARGETS_CMAKE_PATH}" STREQUAL "")
	   message(FATAL_ERROR "Internal beetroot error: FILE_HASH without TARGETS_CMAKE_PATH")
	endif()
	...
	unset(__NO_OP)
   set(__IN_STRING "<#EMPTY#>?!")
#	message(STATUS "Getting Version from '${__TARGETS_CMAKE_PATH}'...")
	build_version_string(__IN_STRING)
	
	if(__NO_OP)
		_set_property_to_db(FILEDB ${__FILE_HASH} BUILD_VERSION "  <#UNDEFINED_FUNCTION#>  ")
		set(${__OUT_FUNCTION_EXISTS} 0 PARENT_SCOPE)
	   set(${__OUT_VERSION_STRING} "" PARENT_SCOPE)
	else()
	   if("${__IN_STRING}" STREQUAL "<#EMPTY#>?!")
	      _get_relative_path("${__TARGETS_CMAKE_PATH}" __NICE_PATH)
	      message(FATAL_ERROR "Beetroot error: function build_version_string(OUT_STRING) defined in ${__NICE_PATH} did not return string. Possible cause: did you put the version string in the parent's scope? Make sure the function ends with `set(\$\{OUT_STRING\} \"$\{MY_VERSION_STRING\}\" PARENT_SCOPE)` where `OUT_STRING` is the name of the first function argument and MY_VERSION_STRING is the name of the variable where the version string is put.")
	   endif()
		_set_property_to_db(FILEDB ${__FILE_HASH} BUILD_VERSION "${__IN_STRING}")
...
macro(_parse_all_file_options __FILE_OPTIONS__REF __OUT_PREFIX)
...
#   message(STATUS "${__PADDING}_parse_all_file_options() ${__FILE_OPTIONS__REF}__LIST: ${${__FILE_OPTIONS__REF}__LIST}")
	cmake_parse_arguments(${__OUT_PREFIX} "${__OPTIONS}" "${__oneValueArgs}" "${__multiValueArgs}" ${${__FILE_OPTIONS__REF}__LIST})
...
#   message(STATUS "${__PADDING}_parse_all_file_options() __PARSED_NO_TARGETS: ${__PARSED_NO_TARGETS} __PARSED_NICE_NAME: ${__PARSED_NICE_NAME}")
endmacro()
...
function(_parse_file_options __TARGETS_CMAKE_PATH __IS_TARGET_FIXED __FILE_OPTIONS__REF __OUT_SINGLETON_TARGETS __OUT_NO_TARGETS __OUT_LANGUAGES __OUT_NICE_NAME __OUT_EXPORTED_VARIABLES __OUT_LINK_TO_DEPENDEE __OUT_DONT_LINK_TO_DEPENDEE __OUT_GENERATE_TARGETS_INCLUDE_LINKPARS __OUT_JOINED_TARGETS)
   ...
   _parse_all_file_options(${__FILE_OPTIONS__REF} __PARSED)
#   message(STATUS "${__PADDING}_parse_file_options() __TARGETS_CMAKE_PATH: ${__TARGETS_CMAKE_PATH} __PARSED_NO_TARGETS: ${__PARSED_NO_TARGETS} __PARSED_NICE_NAME: ${__PARSED_NICE_NAME}")
...
#      message(WARNING "${__PADDING}_parse_file_options() set(${__OUT_NO_TARGETS} 1 PARENT_SCOPE)")
		set(${__OUT_NO_TARGETS} 1 PARENT_SCOPE)
	else()
#	   message(STATUS "${__PADDING}_parse_file_options() ### if(${__PARSED_NO_TARGETS}) did not evaluate to true.")
	...
	if(__PARSED_JOINED_TARGETS)
		message(FATAL_ERROR "Beetroot Error: Template option JOINED_TARGETS put in ${__TARGETS_CMAKE_PATH} has not been implemented yet.")
